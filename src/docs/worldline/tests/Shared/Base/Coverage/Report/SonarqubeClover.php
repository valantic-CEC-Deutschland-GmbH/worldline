...
class SonarqubeClover
{
    ...
    public function process(CodeCoverage $coverage, ?string $target = null, ?string $name = null): string
    {
        ...
        foreach ($report as $item) {
            ...
            $path = ltrim($item->pathAsString(), '/data/');
            $xmlFile->setAttribute('name', $path);
            ...
            $xmlMetrics->setAttribute(
                'elements',
                (string)($item->numberOfMethods() + $item->numberOfExecutableLines() + $item->numberOfExecutableBranches())
            );
            $xmlMetrics->setAttribute(
                'coveredelements',
                (string)($item->numberOfTestedMethods() + $item->numberOfExecutedLines() + $item->numberOfExecutedBranches())
            );
